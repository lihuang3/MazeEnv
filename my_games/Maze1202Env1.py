...
class Maze1202Env1(core.Env):
  ...
  def _build_robot(self):
    ...
    for i in range(self.robot_num):
      ...
      col[self.robot[i]] - 1:col[self.robot[i]] + 1] = robot_marker * np.ones([2, 2])
  ...
  def step(self, action):
    ...
    for i in range(self.robot_num):
      ...
      self.loc[i, 1] - 1:self.loc[i, 1] + 1] = robot_marker * np.ones([2, 2])
