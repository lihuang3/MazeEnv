...
class Maze1204Env(core.Env):
    ...
    def get_reward(self):
        ...
        if max_cost_agent <= self.goal_range:
          ...
          reward += 4
        elif max_cost_agent <= 24 * self.goal_range and not self.reward_grad[7]:
          self.reward_grad[7] = 1
          reward += 2                 
        elif max_cost_agent <= 28 * self.goal_range and not self.reward_grad[8]:
        ...
        if cost_to_go <= self.goal_range * self.robot_num and not self.reward_grad[20]:
            ...
            reward += 2
        elif cost_to_go <= 24 * self.goal_range * self.robot_num and not self.reward_grad[27]:
