...
class Maze1203Env3(core.Env):
    ...
    def get_reward(self):
        ...
        if cost_to_go <= self.goal_range * self.robot_num and not self.reward_grad[20]:
            ...
            reward += 2            
        elif cost_to_go <= 6 * self.goal_range * self.robot_num and not self.reward_grad[25]:
            self.reward_grad[25] = 1
            reward += 2
        elif cost_to_go <= 8 * self.goal_range * self.robot_num and not self.reward_grad[26]:
            self.reward_grad[26] = 1
            reward += 2
        elif cost_to_go <= 10 * self.goal_range * self.robot_num and not self.reward_grad[27]:
