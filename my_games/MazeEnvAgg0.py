...
class MazeEnvAgg0(MazeEnv):
  ...
  def _build_robot(self):
    ...
    self.init_state_img = np.copy(self.state_img)
    self.agg_area_init = np.sum(self.init_state_img>0)
  ...
  def step(self, action):
    ...
    self.agg_rate = float(self.agg_area)/self.agg_area_init
