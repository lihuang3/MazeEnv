...
for i in range(tmp_maze.shape[0]):
    tmp_maze[i,:] = mazeData[int(i/scaling_ratio),:]

for i in range(scaled_maze.shape[1]):
    scaled_maze[:,i] = tmp_maze[:,int(i/scaling_ratio)]
...
np.savetxt('scaled_maze{}_clcostmap1.csv'.format(scaling_ratio), cl_costMap, fmt = '%3d')
...
tree = spatial.KDTree(list(zip(cl_x.ravel(), cl_y.ravel())))
...
np.savetxt('scaled_maze{}_costmap1.csv'.format(scaling_ratio), costMap, fmt = '%3d')
