...
class Maze0122Env1(core.Env):
    ...
    def _load_data(self, data_directory):
        filename = 'map0122'
        self.raw_img = plt.imread(data_directory + '/'+filename+'.png')
    ...
    def render_config(self, loc, dot_size=20):
        ...
        # Draw goal range
        circle = plt.Circle((float(h)/h1*self.goal[1], float(w)/w1*self.goal[0]), 40, color='red', linewidth=3, fill=False)
        plt.gcf().gca().add_artist(circle)
        # Configure robot density
        xs, ys = np.copy(float(h)/h1*loc[:,1]), np.copy(float(w)/w1*loc[:,0])
        xy = np.vstack([xs, ys])
    ...
    def render(self, mode = 'human'):
        for i in range(4):
            plt.gcf().clear()
            fig = plt.gcf()
            fig.set_size_inches(12.671, 10.00)
            loc = self.loc_hist[i]
            self.render_config(loc, dot_size=30)
            if self.save_frame:
